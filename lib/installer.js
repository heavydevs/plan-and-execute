...
const WINDOWS_STORE_ALIAS_PATTERN = /Python was not found|Microsoft Store/i;

/**
 * True when a spawnSync result means "this command does not exist here".
 * Covers ENOENT, Node's EINVAL for .cmd/.bat shims spawned without a shell,
 * cmd.exe's 9009, and the Windows Store python alias that exits 9009 with a
 * "Python was not found" message.
 */
export function isMissingCommand(result) {
  if (!result) return true;
  if (['ENOENT', 'EINVAL'].includes(result.error?.code)) return true;
  if (result.status === WINDOWS_COMMAND_NOT_FOUND) return true;
  const output = `${result.stdout ?? ''}\n${result.stderr ?? ''}`;
  return WINDOWS_STORE_ALIAS_PATTERN.test(output);
}

/**
 * Resolve a bare command through PATH/PATHEXT on Windows with `where.exe`, so
 * npm shims (claude.cmd, codex.cmd, agy.cmd) can be probed without matching
 * cmd.exe's localized "not recognized" message. Returns null when absent.
 */
function resolveWindowsCommand(command) {
  const located = spawnSync('where.exe', [command], { encoding: 'utf8', timeout: 5000, windowsHide: true });
...
export function probeCommand(command, args = ['--version']) {
  const options = { encoding: 'utf8', timeout: 5000, windowsHide: true };
  let result = spawnSync(command, args, options);
  if (process.platform === 'win32' && ['ENOENT', 'EINVAL'].includes(result.error?.code)) {
    const resolved = resolveWindowsCommand(command);
    if (!resolved) return null;
    // .cmd/.bat shims can only run through a shell; the arguments are fixed
    // literals, so shell interpolation is not a concern.
    const isShim = /\.(cmd|bat)$/i.test(resolved);
    result = isShim
      ? spawnSync(`"${resolved}"`, args, { ...options, shell: true })
