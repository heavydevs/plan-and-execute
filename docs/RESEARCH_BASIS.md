...
Operational consequence: run one short, read-only probe per mapped service at validation start and at a default 60-second interval, recording PID/liveness, probe result, timestamps, and a bounded diagnostic excerpt. Do not automatically start, stop, repair, or expose management endpoints on project services.
