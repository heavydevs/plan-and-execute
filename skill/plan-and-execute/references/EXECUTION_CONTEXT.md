...
progressive execution-context decision
          |
          v
fresh plan review, including contexts_minimal
          |
          v
schema-v3 plan creation and validation
```
...
Schema v3 requires an explicit `execution_context` decision even when no files are created:
...
3. read no unassigned context file or other planning artifact;
4. never edit context files;
5. report the exact plan-relative paths in `context_files_read`.

The orchestrator rejects a completion report when the declared reads do not exactly match the task assignment. This prevents silent omission and accidental context leakage.

Fresh worker processes still provide the main context isolation. Progressive files preserve only the small amount of cross-task knowledge that would otherwise need to be rediscovered or copied into every task definition.
